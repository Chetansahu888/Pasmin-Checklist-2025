...
function AccountDataPage() {
  ...
  // Update filteredAccountData calculation using useMemo
  const filteredAccountData = useMemo(() => {
    const query = debouncedSearchTerm.toLowerCase().trim();
    const data = [...accountData];
    
    if (!query) return data.sort(sortDateWise);
    ...
    return data
        ...
        .sort(sortDateWise)
  }, [accountData, debouncedSearchTerm]);

  // Update filteredHistoryData calculation to include member and date filtering using useMemo
  ...
  }, [historyData, debouncedSearchTerm, selectedMembers, startDate, endDate]);
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-3xl font-bold tracking-tight bg-gradient-to-r from-blue-600 to-blue-700 bg-clip-text text-transparent">
                {showHistory ? "Admin Record Explorer" : "Admin Workspace"}
              </h1>
              <p className="text-sm text-slate-600 mt-1">
                {showHistory ? "Comprehensive history of all admin task submissions" : "Manage and track your daily administrative tasks"}
              </p>
            </div>
          </div>
            ...
            {!showHistory ? (
              ...
              selectedHistoryItems.length > 0 && (
                <button
                  ...
                  className="flex-1 md:flex-none flex items-center justify-center gap-2 rounded-xl py-3 px-6 bg-gradient-to-r from-green-600 to-green-700 text-white font-semibold hover:from-green-700 hover:to-green-800 transition-all shadow-md"
                >
        ...
        <div className="rounded-2xl border border-slate-200 shadow-lg bg-white overflow-hidden">
          <div className="bg-gradient-to-r from-slate-50 to-slate-100 p-4 border-b border-slate-200 flex flex-wrap items-center gap-4">
            ...
            <div className="flex-1 min-w-[200px] relative">
              ...
              <input
                type="text"
                placeholder={showHistory ? "Search history records..." : "Quick search transactions..."}
                value={searchTerm}
          ...
          {showHistory && membersList.length > 0 && (
            <div className="p-4 bg-white border-b border-slate-100">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Filter by Member</span>
              </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-bold text-slate-800 leading-none">
                  {showHistory ? "Archive Submission Logs" : "Current Task Queue"}
                </h2>
                <p className="text-xs text-slate-500 mt-1">
                  {showHistory 
                    ? `Showing ${filteredHistoryData.length} archived submissions` 
                    : `Active processing for ${filteredAccountData.length} pending items`
                  }
                </p>
              </div>
            </div>
