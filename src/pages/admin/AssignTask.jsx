...
export default function AssignTask() {
  ...
  const fetchMasterSheetOptions = async () => {
    try {
      const masterSheetId = "1hlFNmLJ_qY7LMpN0pP-N3KWdf0_JRwFvfqWjjLyCDb8";
  ...
  const getLastTaskId = async (sheetName) => {
    try {
      const url = `https://docs.google.com/spreadsheets/d/1hlFNmLJ_qY7LMpN0pP-N3KWdf0_JRwFvfqWjjLyCDb8/gviz/tq?tqx=out:json&sheet=${encodeURIComponent(
        sheetName
      )}`;
  ...
  const fetchWorkingDays = async () => {
    try {
      const sheetId = "1hlFNmLJ_qY7LMpN0pP-N3KWdf0_JRwFvfqWjjLyCDb8";
...
const handleSubmit = async (e) => {
  ...
  try {
    ...
    await fetch(
      "https://script.google.com/macros/s/AKfycbyRbhgAN0TfMUWOgZ1UPiOAsVyUrj7aDM0hOeybHvB-K7NniRVhwhH3foVs5l4u2N2z/exec",
      {
        method: "POST",
        body: formPayload,
        mode: "no-cors",
